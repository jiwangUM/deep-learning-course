...
class WordEmbedding(nn.Module):
    def __init__(self, GloVe, hidden_size, num_layer=1):
        ...
        self.rnn = nn.RNN(GloVe.size(1), hidden_size, num_layer)
    ...
    def forward(self, x):
        ...
        _, h_n = self.rnn(x) #self.rnn(x, h0) if h0 is not given, default 0
...
#    device = torch.device('cpu')
    ...
    hidden_dim = 50 #Rnn hidden layer
    num_layer = 2
    ...
    with open('predictions_q4.txt', 'w') as result_file:
        with torch.no_grad():
