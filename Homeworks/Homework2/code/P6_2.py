...
class WordEmbedding(nn.Module):
    def __init__(self, max_length, vocab_size, embedding_size=128, pool_size=16):
        ...
        self.fc = nn.Sequential(
            nn.Linear(int(max_length * embedding_size/pool_size), 1),
            nn.Sigmoid()
        )
...
#    device = torch.device('cpu')
    ...
    net = WordEmbedding(max_length = max_length, vocab_size = len(word_to_idx)+1, pool_size=8).to(device)
    ...
    with open('predictions_q2.txt', 'w') as result_file:
        with torch.no_grad():
