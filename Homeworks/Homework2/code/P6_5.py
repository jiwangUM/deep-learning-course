...
class WordEmbedding(nn.Module):
    def __init__(self, GloVe, hidden_size, num_layer=1):
        super(WordEmbedding, self).__init__()
        self.train_acc_history = []
        self.test_acc_history = []
        self.dev_acc_history = []
        
        self.embed = nn.Embedding.from_pretrained(GloVe, freeze=False)
        self.lstm = nn.LSTM(GloVe.size(1), hidden_size, num_layer)
        self.fc = nn.Sequential(
            nn.Linear(hidden_size*num_layer, 1),
            nn.Sigmoid()
        )

    def forward(self, x):
        x = self.embed(x) #embeding input dim: (N, T, D)
        x = x.permute(1, 0, 2) #nn.RNN required input dim: (T, N, D)
        _, (h_n, _) = self.lstm(x) #self.rnn(x, h0) if h0 is not given, default 0
        h_n = h_n.permute(1, 0, 2) #nn.RNN return dim: (num_layer, N, D)
#        h_n = h_n.view(h_n.size(0), -1) #view doens't work, don't know why
...
#    device = torch.device('cpu')
    ...
    hidden_dim = 20 #Rnn hidden layer
    num_layer = 1
    ...
    net = WordEmbedding(GloVe=GloVe, hidden_size=hidden_dim, num_layer=num_layer).to(device)
    ...
    with open('predictions_q5.txt', 'w') as result_file:
        with torch.no_grad():
